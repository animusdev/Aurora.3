					...
					if(scanner.cache.unique)
						...
						if(!dbcon.IsConnected())
							...
							*/
							...
							var/DBQuery/query = dbcon.NewQuery("INSERT INTO erro_library (author, title, content, category, uploadtime, uploader) VALUES ('[sqlauthor]', '[sqltitle]', '[sqlcontent]', '[sqlcategory]', NOW(), '[sqlckey]')")
							if(!query.Execute())
								usr << query.ErrorMsg()
							else
