...
/client/proc/warn(warned_ckey)
	...
	if (C)
		...
		var/DBQuery/lookup_query = dbcon.NewQuery("SELECT ip, computerid FROM erro_player WHERE ckey = :ckey:")
	...
	var/DBQuery/insert_query = dbcon.NewQuery("INSERT INTO erro_warnings (id, time, severity, reason, notes, ckey, computerid, ip, a_ckey, a_ip, a_computerid) VALUES (null, Now(), :warning_severity:, :warning_reason:, :warning_notes:, :warned_ckey:, :warned_computerid:, :warned_ip:, :a_ckey:, :a_ip:, :a_computerid:)")
...
/client/verb/warnings_check()
	...
	var/DBQuery/search_query = dbcon.NewQuery("SELECT id, time, severity, reason, a_ckey, acknowledged, expired FROM erro_warnings WHERE visible = 1 AND (ckey = :ckey: OR computerid = :computer_id: OR ip = :address:) ORDER BY time DESC;")
...
/client/proc/warnings_acknowledge(warning_id)
	...
	var/DBQuery/query = dbcon.NewQuery("UPDATE erro_warnings SET acknowledged = 1 WHERE id = :warning_id:;")
...
/client/proc/warnings_gather()
	...
	var/DBQuery/expire_query = dbcon.NewQuery("SELECT id FROM erro_warnings WHERE (acknowledged = 1 AND expired = 0 AND DATE_SUB(CURDATE(),INTERVAL 3 MONTH) > time) AND (ckey = :ckey: OR computerid = :computer_id: OR ip = :address:)")
	expire_query.Execute(client_details)
	while (expire_query.NextRow())
		var/warning_id = text2num(expire_query.item[1])
		var/DBQuery/update_query = dbcon.NewQuery("UPDATE erro_warnings SET expired = 1 WHERE id = :warning_id:")
		update_query.Execute(list("warning_id" = warning_id))
		count_expire++

	var/DBQuery/query = dbcon.NewQuery("SELECT id FROM erro_warnings WHERE (visible = 1 AND acknowledged = 0 AND expired = 0) AND (ckey = :ckey: OR computerid = :computer_id: OR ip = :address:)")
...
/datum/admins/proc/warning_panel(var/adminckey = null, var/playerckey = null)
	...
	if (adminckey || playerckey)
		...
		var/DBQuery/search_query = dbcon.NewQuery("SELECT id, time, severity, reason, notes, ckey, a_ckey, acknowledged, expired, edited, lasteditor, lasteditdate FROM erro_warnings WHERE visible = 1 [paramone] [paramtwo] ORDER BY time DESC;")
...
/proc/warningsEdit(var/warning_id, var/warning_edit)
	...
	var/DBQuery/initial_query = dbcon.NewQuery("SELECT ckey, reason, notes FROM erro_warnings WHERE id = :warning_id:")
	...
	switch (warning_edit)
		if ("delete")
			if(alert("Delete this warning?", "Delete?", "Yes", "No") == "Yes")
				var/DBQuery/deleteQuery = dbcon.NewQuery("UPDATE erro_warnings SET visible = 0 WHERE id = :warning_id:")
		...
		if ("editReason")
			...
			var/DBQuery/reason_query = dbcon.NewQuery("UPDATE erro_warnings SET reason = :new_reason:, edited = 1, lasteditor = :a_ckey:, lasteditdate = NOW() WHERE id = :warning_id:")
		...
		if("editNotes")
			...
			var/DBQuery/notes_query = dbcon.NewQuery("UPDATE erro_warnings SET notes = :new_notes:, edited = 1, lasteditor = :a_ckey:, lasteditdate = NOW() WHERE id = :warning_id:")
