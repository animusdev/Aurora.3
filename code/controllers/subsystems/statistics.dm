...
/datum/controller/subsystem/statistics/fire()
	...
	if (config.sql_enabled && config.sql_stats)
		...
		if(!dbcon.IsConnected())
			...
			var/DBQuery/query = dbcon.NewQuery("INSERT INTO `erro_population` (`playercount`, `admincount`, `time`) VALUES ([playercount], [admincount], '[sqltime]')")
			if(!query.Execute())
...
/datum/controller/subsystem/statistics/Shutdown()
	...
	for(var/datum/feedback_variable/FV in feedback)
		var/sql = "INSERT INTO erro_feedback VALUES (null, Now(), \"[game_id]\", \"[FV.get_variable()]\", [FV.get_value()], \"[FV.get_details()]\")"
